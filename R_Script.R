...
df_train_akshay <- fread('Cleaned_train_additional.csv')
df_test_akshay <- fread('Cleaned_test_additional.csv')

# add in the sentiment analysis from akshay
df$Interaction_Sentiment <- df_train_akshay$Interaction_Sentiment
test_Interaction_Sentiment <- df_test_akshay$Interaction_Sentiment

# setwd("C:\\Users\\vivek_000\\Desktop\\Data Mining\\Airbnb-Project")
#df <-read.csv("airbnb_train_x.csv", header = TRUE)
#test <- read.csv("airbnb_test_x.csv", header = TRUE)
...
#set seed
set.seed(12345) #tried different seeds - 123,42 - both had acc ~ 93% on valid---------------------------
...
# make model
bstair<- xgboost(data=dtrain, max.depth = 14, eta =0.0390,min_child_weight = 2,nthread =8,nrounds = 3000, max_delta_step = 2,colsample_bytree = 0.44, early_stopping_rounds = 400, objective = "binary:logistic",evalmetric = "auc")
